document.addEventListener('DOMContentLoaded', () => {
	const users = [
		{
			"id": 0,
			"userLogo": "./photos/AMIR.jpg",
			"name": "Mon petit prince ",
			"email": "JOUIHRI Mohammed Ami",
		},
		{
			"id": 1,
			"userLogo": "./photos/SOFI.jpg",
			"name": "Ma championne",
			"email": "JOUIHRI SOFIA",
		},
		{
			"id": 2,
			"userLogo": "./photos/INES.jpg",
			"name": "Ma grande fille",
			"email": "JOUIHRI INES",
		},
		{
			"id": 3,
			"userLogo": "./photos/mehdi-hanane.jpeg",
			"name": "Mon petit prince",
			"email": "SULTAN",
...
// insertion des images 
	...
	const photos = [
		{
			...
			"id": 1,
			"title": "",
			"url": "./photos/A1-600.jpg",
			"thumbnailUrl": "./Amir/AA.png"
	...
	const galleryTitleCont = document.querySelector('.app__title')
	const galleryTitle = document.createElement('h1')
	galleryTitle.textContent = 'Users Gallery'
	galleryTitleCont.appendChild(galleryTitle)
	const body = document.querySelector('body')
	const photoGallery = document.querySelector('.photo-gallery')
	const hotoGalleryButton = document.querySelector('.photo-gallery__button')
	hotoGalleryButton.addEventListener('click', () => {
		photoGallery.classList.remove('_active')
		body.classList.remove('_lock')
	})

	const makeAlbumsFromObject = (user) => {
		const userCard = document.createElement('article')
		userCard.className = "user"
		const userInfo = document.createElement('div')
		userInfo.className = "user__info"
		const userInfoImg = document.createElement('img')
		userInfoImg.src = "/Amir/Amir-logo.png"
		userInfo.appendChild(userInfoImg)
		userCard.appendChild(userInfo)
		const logo = document.createElement('div')
		logo.className = "user__logo"
		userCard.appendChild(logo)
		const logoImg = document.createElement('img')
		logoImg.src = user.userLogo
		...
		userMail.className = "user__mail"
		userMail.innerText = user.email
		userCard.appendChild(userMail)
		const minGallery = document.createElement('div')
		minGallery.className = "user__min-gallery min-gallery"
		const minGalleryTitle = document.createElement('div')
		minGalleryTitle.className = "min-gallery__title"
		minGalleryTitle.innerText = "Gallery preview"
		minGallery.appendChild(minGalleryTitle)
		minGallery.appendChild(getMinGallery(user.id))
		const userButtunContainer = document.createElement('div')
		userButtunContainer.className = "user__buttun-container"
		minGallery.appendChild(userButtunContainer)
		userCard.appendChild(minGallery)
		return userCard
	}

	const getMinGallery = (albumId) => {
		const minGalleryBody = document.createElement('div')
		minGalleryBody.className = "min-gallery__body"
		const filteredFoto = albums.filter(album => album.userId === albumId)
		if (!filteredFoto.length) {
			const minGalleryPreview = document.createElement('div')
			minGalleryPreview.className = "min-gallery__preview "
			minGalleryPreview.innerText = 'No albums :- ('
			minGalleryBody.appendChild(minGalleryPreview)
			return minGalleryBody
		}
		filteredFoto.forEach((album) => {
			const minGalleryPreview = document.createElement('div')
			minGalleryPreview.className = "min-gallery__preview"
			minGalleryPreview.innerText = album.title
			minGalleryPreview.id = "albumID_" + album.id
			minGalleryBody.appendChild(minGalleryPreview)

			minGalleryPreview.addEventListener('click', (event) => {
				photoGallery.classList.add('_active')
	...
	users.forEach(user => {
		appRow.appendChild(makeAlbumsFromObject(user))
	})

	const minGalleryItem = document.createElement('div')

	albums.forEach(album => {
		minGalleryItem.appendChild(getMinGallery(album))
	})

	const photoGalleryBody = document.querySelector('.photo-gallery__body')
	const renderPhotos = (event) => {
		photoGalleryBody.innerHTML = ""
		let elem = event.target
		let id = +elem.id.split("_")[1]
		const filteredFoto = photos.filter((photo) => photo.albumId === id)
		const photoGalleryItems = document.createElement('div')
		photoGalleryItems.className = "photo-gallery__items"
		filteredFoto.forEach(photo => {
			const photoGalleryItem = document.createElement('div')
			photoGalleryItem.className = "photo-gallery__item"
			const photoGalleryImgCont = document.createElement('div')
			photoGalleryImgCont.className = "photo-gallery__img-container"
			const photoGalleryImg = document.createElement('img')
			photoGalleryImg.className = "photo-gallery__img"
			photoGalleryImg.src = photo.thumbnailUrl
			const photoGalleryTitle = document.createElement('div')
			photoGalleryTitle.className = "photo-gallery__title"
			photoGalleryTitle.innerText = photo.title
			photoGalleryImgCont.appendChild(photoGalleryImg)
			photoGalleryItem.appendChild(photoGalleryImgCont)
			photoGalleryItem.appendChild(photoGalleryTitle)
			photoGalleryItems.appendChild(photoGalleryItem)
			photoGalleryItem.addEventListener('click', () => {
				if (!photoGalleryItem.classList.contains('_show')) {
					photoGalleryItem.classList.add('_show')
					photoGalleryImg.src = photo.url
				}
				else{
					photoGalleryItem.classList.remove('_show')
					photoGalleryImg.src = photo.thumbnailUrl
				}
			})

		})
		photoGalleryBody.appendChild(photoGalleryItems)
